...
function getEncodeWriteSnippet(ctx, field) {
    ...
    if ((0, types_1.isEnum)(field) && options.stringEnums) {
        ...
        writer.uint32(${tag}).${(0, types_1.toReaderCall)(field)}(${place})`;
            case "uint64":
            case "fixed64":
        ...
        const tag = ((field.number << 3) | (0, types_1.basicWireType)(field.type)) >>> 0;
        if ((0, types_1.isLong)(field) && options.forceLong === options_1.LongOption.LONG) {
            return (place) => (0, ts_poet_1.code) `writer.uint32(${tag}).${(0, types_1.toReaderCall)(field)}(${place}.toString())`;
        }
    ...
    messageDesc.field.forEach((field) => {
        ...
        `;
          ...
          `);
                ...
                const tag = ((field.number << 3) | 2) >>> 0;
                const rhs = (x) => ((0, types_1.isLong)(field) && options.forceLong === options_1.LongOption.LONG ? `${x}.toString()` : x);
    ...
    `);
        function getEncodeSnippet(ctx, field) {
            ...
            if ((0, types_1.isEnum)(field) && options.stringEnums) {
                ...
                const type = (0, types_1.basicTypeName)(ctx, field, { keepValueType: true });
                return (place) => (0, ts_poet_1.code) `${type}.encode(${utils.toProtoObjectId}(${place}), writer.fork()).join()`;
            }
            else if ((0, types_1.isTimestamp)(field) &&
                (options.useDate === options_1.DateOption.DATE ||
                    options.useDate === options_1.DateOption.STRING ||
                    options.useDate === options_1.DateOption.STRING_NANO)) {
                const type = (0, types_1.basicTypeName)(ctx, field, { keepValueType: true });
                return (place) => (0, ts_poet_1.code) `${type}.encode(${utils.toTimestamp}(${place}), writer.fork()).join()`;
            }
            else if ((0, types_1.isValueType)(ctx, field)) {
                const maybeTypeField = (0, options_1.addTypeToMessages)(options) ? `$type: '${field.typeName.slice(1)}',` : "";
                const type = (0, types_1.basicTypeName)(ctx, field, { keepValueType: true });
                ...
                return (place) => (0, ts_poet_1.code) `${type}.encode(${wrappedValue(place)}, writer.fork()).join()`;
            }
            else if ((0, types_1.isMessage)(field)) {
                const type = (0, types_1.basicTypeName)(ctx, field);
                if (field.type == ts_proto_descriptors_1.FieldDescriptorProto_Type.TYPE_GROUP) {
                ...
                return (place) => (0, ts_poet_1.code) `${type}.encode(${place}, writer.fork()).join()`;
            }
            else {
                throw new Error(`Unhandled field ${field}`);
            }
        ...
        `);
                ...
                const rhs = (x) => (0, types_1.isLong)(extension) && ctx.options.forceLong === options_1.LongOption.LONG ? `${x}.toString()` : x;
                chunks.push((0, ts_poet_1.code) `
          const writer = new ${BinaryWriter};
          writer.fork();
          for (const v of value) {
...
function generateFromJson(ctx, fullName, fullTypeName, messageDesc) {
    ...
    if (fullTypeName === "google.protobuf.Duration" && ctx.options.useDuration === options_1.DurationOption.STRING) {
        return (0, ts_poet_1.code) `
...
function generateCanonicalToJson(fullName, fullProtobufTypeName, { useOptionals, useNullAsOptional, useDuration }) {
  ...
  `;
    ...
    if (fullProtobufTypeName === "google.protobuf.Duration" && useDuration === options_1.DurationOption.STRING) {
        return (0, ts_poet_1.code) `
...
function generateFromPartial(ctx, fullName, messageDesc) {
    ...
    messageDesc.field.forEach((field) => {
        ...
        if (currentSwitchTarget && !(0, types_1.isWithinOneOfThatShouldBeUnion)(options, field)) {
